readme_content = """# Cluster K3s Multizona na AWS com Terraform e Testes de Resiliência (Self-Healing)
...
Abaixo estão detalhadas as etapas de validação do cluster, utilizando capturas de tela reais extraídas diretamente do ambiente de homologação. Os arquivos de imagem estão localizados no diretório `docs/images/`.
...
* **Arquivo:** `docs/images/Captura de tela 2026-07-02 201434.png`
* **Descrição:** Confirmação visual da página padrão do Nginx respondendo de forma estável no navegador.
...
* **Arquivo:** `docs/images/Captura de tela 2026-07-02 201539.png`
* **Descrição:** Saída do comando `kubectl get nodes` exibindo o nó master (`ip-10-0-101-86`) e o nó worker (`ip-10-0-102-198`) em estado `Ready`.
...
* **Arquivo:** `docs/images/Captura de tela 2026-07-02 201609.png`
* **Descrição:** Execução do comando `kubectl get pods -o wide` comprovando o agendamento de pods distintos em instâncias EC2 fisicamente separadas no cluster.
...
* **Arquivo:** `docs/images/Captura de tela 2026-07-02 202111.png`
* **Descrição:** Histórico de comandos demonstrando a remoção do pod e a reação imediata do Kubernetes Controller Manager, que identificou a perda e iniciou a criação de um pod substituto (`meu-app-web-7bc846868-b4w6t`) em apenas 10 segundos.
...
* **Arquivo:** `docs/images/Captura de tela 2026-07-02 202231.png`
* **Descrição:** Cluster estabilizado com o novo pod completamente operacional (`Running`) e integrado à malha de serviços, consolidando o sucesso do teste de resiliência automatizada.
...
2. **Configuração do Cluster:** Conecte-se via SSH à instância principal para instalar o K3s Server. Utilize o token gerado para configurar o K3s Agent no nó worker secundário.
